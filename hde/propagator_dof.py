...
__all__ = ['Propagator_dof', 'get_mixture_loss_func_dof']
...
def get_mixture_loss_func_dof(output_dim, num_mixes):
...
def get_mixture_fun_dof(output_dim, num_mixes):
...
class Propagator_dof(BaseEstimator, TransformerMixin):

    def __init__(self, input_dim, dof_dim, n_components=2, lag_time=100, batch_size=1000, 
        ...
        model.add(MDN(self.dof_dim, n_components))
        model.compile(loss=get_mixture_loss_func_dof(self.dof_dim, n_components), 
    ...
    def fit(self, X, y=None): 
        x0, xt = self._create_dataset(X, dof_dim=self.dof_dim)
    ...
    def _create_dataset(self, data, dof_dim, lag_time=None):
        '''Inputs are include dof at x0 and srvs at xt
           Outputs are only dof at xt
           srv_dim determines seperation between srv and dof modes'''
        ...
        if type(data) is list: 
            ...
            x_tt = []
            for item in data:
                dof_t0 = item[:-lag_time:, -dof_dim:]
                dof_tt = item[lag_time:, -dof_dim:]
                srv_tt = item[lag_time:, :-dof_dim]
                
                x_t0.append(np.concatenate([dof_t0, srv_tt], axis=1))
                x_tt.append(dof_tt)
            ...
            x_t0 = np.concatenate([dof_t0, srv_tt], axis=1)
            x_tt = dof_tt
    ...
    def propagate(self, x0, xp, n_steps=1):
        '''Propagation proceeds as a function of x0 dof and xt srv'''
        
        print(x0.shape, xp.shape)
        mix_fun = get_mixture_fun_dof(self.dof_dim, self.n_components)
        
        def body(loop_counter, x, xp, x_out):
            y = self.model(x)
            ...
            # periodic wrap any values outside (0, 1) 
            xn = tf.math.floormod(xn, tf.ones(xn.shape))
            
            # add next srv component to xn
            xt_srv = tf.expand_dims(xp[loop_counter], axis=0)
            xn = tf.concat([xn, xt_srv], axis=1)
            x_out = x_out.write(loop_counter, xn)
            
            return [loop_counter + 1, xn, xp, x_out]
    
        def cond(loop_counter, x, xp, x_out):
        ...
        with tf.variable_scope('propagate', reuse=tf.AUTO_REUSE):
            ...
            result = tf.while_loop(cond, body, [loop_counter, x0, xp, x_out])
            traj = result[-1].stack()
