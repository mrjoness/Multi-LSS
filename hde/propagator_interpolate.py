...
def get_mixture_loss_func(output_dim, num_mixes):
    ...
    def loss_func(y_true, y_pred):
        ...
        print("Using Interpolated loss func", output_dim, num_mixes)
        print(y_true)
        print(y_pred)
...
class Propagator(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, input_dim, n_components=2, lag_time=100, batch_size=1000, 
        ...
        print(get_mixture_loss_func)
        
        # subtract 1 from each input_dim to account for temp
        model.add(MDN(input_dim-1, n_components))
        model.compile(loss=get_mixture_loss_func(input_dim-1, n_components), optimizer=keras.optimizers.Adam(lr=learning_rate))
    ...
    def fit(self, X, y=None): 
        ...
        # don't include last mode in x_t:
        xt = xt[:, :-1]
        
        print('Shape create_dataset', x0.shape, xt.shape)
    ...
    def propagate(self, x0, n_steps=1):
        ...
        # subtract 1 from mix_fun output
        mix_fun = get_mixture_fun(self.input_dim - 1, self.n_components)
        ...
        def body(loop_counter, x, x_out):
            ...
            # append temp instead of predicting it
            temp = tf.reshape(x[:, -1], [1, 1])
            xn = tf.concat([xn, temp], axis=1)
